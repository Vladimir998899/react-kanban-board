...
import styles from "./recast.module.css";

export const Recast = ({
  backlog, moveTask, card, ready, progress, activeSelect, setSelect,
}) => {
  ...
  if (card.title === "Ready") {
    ...
    if (ready.length === 0 && activeSelect.progress) {
      content = (
        <button className={css.add} key={card.id}
          onClick={handleSelect} disabled>
          + add card
        </button>
      );
      ...
      content = (
        <select
          className={styles.select} onChange={(e) => moveTask(e.target.value, ready)}
          multiple>
          {ready.map((halt) => (
            <option className={styles.option} key={halt.id} value={halt.name}>
              {halt.name}
            </option>
          ))}
        </select>
    ...
    if (progress.length === 0 && activeSelect.finished) {
      ...
      content = (
        <select
          className={styles.select} onChange={(e) => moveTask(e.target.value, progress)}
          multiple>
          {progress.map((halt) => (
            <option className={styles.option} key={halt.id} value={halt.name}>
              {halt.name}
