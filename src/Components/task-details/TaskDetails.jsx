...
const TaskDetails = ({
  ...
  if (!edit && task !== 0) {
    ...
    content = (
      <div className={css.form}>
        <input className={css.input} type="text" value={title}
          onChange={(e) => handleTitle(e.target.value)} autoFocus={true} />{" "} <br />
        <textarea
          className={css.textarea} value={description}
    ...
    content = (
      <h1 className={css.empty}>Task with ID <em>{taskId}</em> was not found</h1>
    );
  }
  return (
    <div className={css.container}>
      <Link title="back" className={css.back} to="/">{" "}<img src={back} alt="" /></Link>{" "}<br />
      {content}
    </div>
  );
};
